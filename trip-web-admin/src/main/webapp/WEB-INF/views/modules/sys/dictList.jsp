...
<body>
	...
	<form:form id="searchForm" modelAttribute="sysDict" action="${ctx}/sys/dict/" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="sysDict">
			<tr>
				<td>${sysDict.value}</td>
				<td><a href="${ctx}/sys/dict/form?id=${sysDict.id}">${sysDict.label}</a></td>
				<td><a href="javascript:" onclick="$('#type').val('${sysDict.type}');$('#searchForm').submit();return false;">${sysDict.type}</a></td>
				<td>${sysDict.description}</td>
				<td>${sysDict.sort}</td>
				<shiro:hasPermission name="sys:dict:edit"><td>
    				<a href="${ctx}/sys/dict/form?id=${sysDict.id}">修改</a>
					<a href="${ctx}/sys/dict/delete?id=${sysDict.id}&type=${sysDict.type}" onclick="return confirmx('确认要删除该字典吗？', this.href)">删除</a>
    				<a href="<c:url value='/sys/dict/form?type=${sysDict.type}&sort=${sysDict.sort+10}'><c:param name='description' value='${sysDict.description}'/></c:url>">添加键值</a>
