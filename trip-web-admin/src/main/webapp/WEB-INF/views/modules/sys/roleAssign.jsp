...
<body>
	...
	<div class="breadcrumb">
		...
		<script type="text/javascript">
			$("#assignButton").click(function(){
				top.$.jBox.open("iframe:${ctx}/sys/role/usertorole?id=${sysRole.id}", "分配角色",610,$(top.document).height()-240,{
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${userList}" var="sysUser">
			<tr>
				<td><a href="${ctx}/sys/user/form?id=${sysUser.id}">${sysUser.loginName}</a></td>
				<td>${sysUser.name}</td>
				<td>${sysUser.phone}</td>
				<td>${sysUser.mobile}</td>
				<shiro:hasPermission name="sys:role:edit"><td>
					<a href="${ctx}/sys/role/outrole?userId=${sysUser.id}&roleId=${sysRole.id}"
						onclick="return confirmx('确认要将用户<b>[${sysUser.name}]</b>从<b>[${sysRole.name}]</b>角色中移除吗？', this.href)">移除</a>
