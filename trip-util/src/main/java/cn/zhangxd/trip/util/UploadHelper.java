...
public class UploadHelper {
    ...
    public UploadHelper() {
        // 其中images,flashes,medias,files,对应文件夹名称,对应dirName
        // key文件夹名称
        // value该文件夹内可以上传文件的后缀名
        extMap.put("images", "gif,jpg,jpeg,png,bmp");
        extMap.put("flashes", "swf,flv");
    ...
    public String[] uploadFile(HttpServletRequest request) {
        ...
        // 上传
        List<FileItem> fiList = (List<FileItem>) fieldsMap.get(UploadHelper.FILE_FIELDS);
        if (fiList != null) {
    ...
    private String validateFields(HttpServletRequest request) {
        ...
        if (contentType == null || !contentType.startsWith("multipart")) {
            ...
            logger.error("上传目录[" + savePath + "]不存在");
            errorInfo = "上传目录[" + savePath + "]不存在";
        } else if (!uploadDir.canWrite()) {
            logger.error("上传目录[" + savePath + "]没有写权限");
            ...
            if (!saveDirFile.exists()) {
                if (!saveDirFile.mkdirs()) {
                    logger.error("上传目录[" + savePath + "]创建失败");
                    errorInfo = "上传目录[" + savePath + "]创建失败";
                }
            }
            ...
            if (!file.exists()) {
                if (!file.mkdirs()) {
                    logger.error("临时上传目录[" + savePath + "]创建失败");
                    errorInfo = "临时上传目录[" + savePath + "]创建失败";
                }
            }
        }
